# Autogenerated wrapper script for TVM_jll for x86_64-linux-gnu
...
const libtvm_splitpath = ["lib", "libtvm.so"]
...
const libtvm = "libtvm.so"


# Relative path to `libnnvm_compiler`
const libnnvm_compiler_splitpath = ["lib", "libnnvm_compiler.so"]
...
# This must be `const` so that we can use it with `ccall()`
const libnnvm_compiler = "libnnvm_compiler.so"


# Relative path to `libtvm_topi`
const libtvm_topi_splitpath = ["lib", "libtvm_topi.so"]
...
# This must be `const` so that we can use it with `ccall()`
const libtvm_topi = "libtvm_topi.so"


# Relative path to `libtvm_runtime`
const libtvm_runtime_splitpath = ["lib", "libtvm_runtime.so"]
...
# This must be `const` so that we can use it with `ccall()`
const libtvm_runtime = "libtvm_runtime.so"
